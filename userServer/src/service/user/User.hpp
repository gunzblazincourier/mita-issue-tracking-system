...
class UserModel {
 ...
 public:
  ...
  /**
   * @brief Mutable access to the internal users vector (for loading/saving)
   */
...
// Inline implementations (inside include guard to avoid duplicate definitions)
inline User::User() : _id(0), name("") {}
inline User::User(int id) : _id(id), name("") {}

inline int User::GetID() const { return _id; }
inline bool User::operator==(const User& other) const { return _id == other._id; }
inline void User::SetID(int id) { _id = id; }
inline const std::string& User::GetName() const { return name; }
inline void User::SetName(const std::string& n) { name = n; }

// UserModel
inline void UserModel::AddUser(const User& user) { users.push_back(user); }
inline User* UserModel::FindUserByID(int id) {
  auto it = std::find_if(users.begin(), users.end(), [id](const User& u){ return u.GetID() == id; });
  return (it != users.end()) ? &(*it) : nullptr;
}
inline const std::vector<User>& UserModel::GetUsersVector() const { return users; }

inline std::vector<User>& UserModel::GetUsersVector() { return users; }

inline bool UserModel::UpdateUserName(int id, const std::string& newName) {
...
inline bool UserModel::DeleteUserByID(int id) {
